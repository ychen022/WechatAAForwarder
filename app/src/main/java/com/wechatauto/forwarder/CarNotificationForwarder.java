 ...
 *
 * IMPORTANT: Android Auto only surfaces a MessagingStyle notification as a
 * conversation when it carries BOTH a reply action (with RemoteInput) AND a
 * mark-as-read action. Without the reply action, Auto shows "No new messages".
 * WeChat can't actually receive our reply (no inline-reply hook), so the reply is
 * best-effort and reported as undeliverable — but the action must exist for
 * messages to appear/read aloud at all.
 */
